...
const UserProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserDetails = async () => {
      try {
        ...
        setImg(`http://ec2-13-201-227-93.ap-south-1.compute.amazonaws.com/${data.imageUrl}`);
      } catch (error: any) {
        if (error.response?.status === 404) {
          try {
            ...
            setImg(`http://ec2-13-201-227-93.ap-south-1.compute.amazonaws.com/${dataBackup.imageUrl}`);
            toast({
              title: "Required!",
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        "http://ec2-13-201-227-93.ap-south-1.compute.amazonaws.com/api/patient/update",
        {
  ...
  const formFields = [
    ...
    { label: "Patient ID", name: "patientId", icon: ClipboardList, disabled: true },
